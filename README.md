...
- **Re-invoke guard** (`reinvokeGuard`, default `true`): if the agent re-invokes a review-mode tool while an invocation is still pending, the guard catches the new fill's `input` events — the one window before Chromium drops the old reply — cancels the old invocation cleanly via `form.reset()` (snapshotting and restoring every control value so the new fill survives), and emits an `invocation-reinvoked` warning. Don't combine it with a `reset`-event listener that calls `preventDefault()`.
